...
import { OrdemServicoRepository } from '../ordens-de-servico/repositories/ordem-servico.repository';
...
export class AmostraService {
  ...
  async detalhesAmostra(id: number) {
  ...
  async update(
    ...
    if (amostraExists.status === 'FINALIZADA') {
      throw new HttpException(
        'Não é possível alterar esta amostras. O registro já foi finalizadoAmostra Bloqueada. Após a assinatura/validação, as modificações são impedidas para garantir a integridade do laudo.',
